...
-- =========================================================================
-- Fonctions utilitaires (updated_at + helpers de rôle pour les policies RLS)
-- =========================================================================
...
-- (contrôlé par le trigger ci-dessous, pas seulement par la policy).
...
-- Empêche toute modification de `role` hors de la fonction set_user_role (SECURITY DEFINER).
...
-- exclusivement par la fonction get_or_create_efs_link (SECURITY DEFINER) et l'incrément
-- du compteur par la route de redirection (service_role, hors RLS par nature).
...
-- SECURITY DEFINER (set_user_role, trg_log_hard_delete) qui contournent RLS par conception.
